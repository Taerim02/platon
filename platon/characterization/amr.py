...
"""Search for AMR genes in the metagenomic module of platon."""
            ...
            if hit.included:
                ...
                if f'{contig_id}_{orf_id}' not in hits_set:
                    ...
                    with open(args.output, "a", newline='') as fh:
                        ...
                        if is_empty:
                            writer.writeheader()
                        tsv_row = {"contig":contig_id, "type": hit_info['type'], "bitscore":hit_info["bitscore"], "hmm-id":hit_info["hmm-id"], "evalue":hit_info["evalue"],  "orf_id": orf_id}
