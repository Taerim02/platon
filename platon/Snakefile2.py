...
file_name = pf.get_base_name(str(main_fasta_file)) 

ncbi_amr = cfg.db_path.joinpath('ncbifam-amr')
ncbi_rep = cfg.db_path.joinpath('replication')
ncbi_mob = cfg.db_path.joinpath('mobilization')
ncbi_conj = cfg.db_path.joinpath('conjugation')
...
def get_inputs(wildcards):
    ...
    files = []
    for t in faa:
        files.extend(expand("tmp/function/{txt}/{file_name}_{i}_{txt}.txt", file_name=file_name, i=faa_chunk_list, txt=t))
    for t in fasta:
        files.extend(expand("tmp/function/{txt}/{file_name}_{i}_{txt}.txt", file_name=file_name, i=fasta_chunk_list, txt=t))
    return files
...
rule search_amr_genes:
    ...
    output:
        amr = 'tmp/function/amr/{file_name}_{i}_amr.txt'   #str(cfg.db_path.joinpath('ncbifam-amr'))
    params:
        db_path = cfg.db_path.joinpath('ncbifam-amr'),
        name = file_name
    shell:
        'python amr.py --name {params.name} --db_path {params.db_path} --output {output.amr} {input.faa_file}'
...
rule search_replication_genes: 
    input:
        faa_file = 'tmp/{file_name}_{i}_filtered.faa',
    output:
        rep = 'tmp/function/rep/{file_name}_{i}_rep.txt'   #str(cfg.db_path.joinpath('ncbifam-amr'))
    params:
        db_path = cfg.db_path.joinpath('replication'),
        name = file_name
    shell:
        'python rep.py --name {params.name} --db_path {params.db_path} --output {output.rep} {input.faa_file}'
...
rule search_mobilization_genes:
    input:
        faa_file = 'tmp/{file_name}_{i}_filtered.faa',
    output:
        mob = 'tmp/function/mob/{file_name}_{i}_mob.txt'   #str(cfg.db_path.joinpath('ncbifam-amr'))
    params:
        db_path = cfg.db_path.joinpath('mobilization'),
        name = file_name
    shell:
        'python mob.py --name {params.name} --db_path {params.db_path} --output {output.mob} {input.faa_file}'
...
rule search_conjugation_genes:
    input:
        faa_file = 'tmp/{file_name}_{i}_filtered.faa',
    output:
        conj = 'tmp/function/conj/{file_name}_{i}_conj.txt'   #str(cfg.db_path.joinpath('ncbifam-amr'))
    params:
        db_path = str(cfg.db_path.joinpath('conjugation')),
        name = file_name
    shell:
        'python conj.py --name {params.name} --db_path {params.db_path} --output {output.conj} {input.faa_file}'
...
rule test_circularity:
    ...
    params:
        ...
        name = file_name
    shell: 
        'python test_circularity.py --name {params.name} --tmpdir {params.tmpdir} --output {output.cir} {input.fasta_file}'
...
rule search_rrnas:
    ...
    shell: 
        ...
        python rrnas_result.py --name {params.name} --output {output.rrnas} {input.fasta_file} {params.rrnas} 
        """
...
rule search_inc_type:
    ...
    shell:
        ...
        python inc_result.py --name {params.name} --output {output.inc} {input.contig_file} {params.inc}
        """
...
rule search_orit_sequences:
    ...
    shell:
        ...
        python orit_result.py --name {params.name} --output {output.orit} {input.contig_file} {params.orit}
        """
...
rule search_reference_plasmids:  
    ...
    shell:
        ...
        python ref_result.py --name {params.name} --output {output.ref} {input.contig_file} {params.ref}
        """
